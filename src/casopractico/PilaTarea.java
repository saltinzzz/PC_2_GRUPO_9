...
public class PilaTarea {
    ...
    public class PilaDeTareas {
        ...
        // Método mostrar en PilaDeTareas
        ...
        public Tarea buscar(String nombre) {
            for (int i = 0; i <= topeAlta; i++) {
                if (pilaAlta[i].getNombre().equalsIgnoreCase(nombre)) {
                    return pilaAlta[i];
                }
            }
            for (int i = 0; i <= topeMedia; i++) {
                if (pilaMedia[i].getNombre().equalsIgnoreCase(nombre)) {
                    return pilaMedia[i];
                }
            }
            for (int i = 0; i <= topeBaja; i++) {
                if (pilaBaja[i].getNombre().equalsIgnoreCase(nombre)) {
                    return pilaBaja[i];
                }
            }
